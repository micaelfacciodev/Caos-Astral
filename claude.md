...
- **Quíron**: fora da astronomy-engine. Kepleriana de dois corpos
  (elementos JPL SBDB, época 2021-Jul-01). Sem correção de perturbação, aceitável pra uso pessoal.
...
- **Identidade visual**: decisão do usuário foi **padronizar o layout
  ...
  vermelho/areia, EB Garamond) ainda pendentes de unificação visual, funcionais, mas com identidade própria por ora.
...
- **Deriva** (`deriva.html` + `aura_flow.html`) trazida do site pessoal de
  ...
  (rorschach, stars, wormhole, plasma, mandala) estavam quebrados, existiam como arquivos soltos (`hyperspace_flow.html`,
...
- **Pendência em aberto:** admin hub pra upload de arte de símbolos
  astrológicos (efeito "flash decor" trazido do site de tatuagem), **implementado** em `admin-simbolos.html` + `assets/flash-decor.js`,
...
**Sessão de 29/07, correção do bug "kit vazio":**
- Diagnóstico confirmado: `ritual-de-entrada.html` nunca chamava
  `compute-natal-chart`, os valores de núcleo/fome/máscara vinham de
  arrays `FAKE_NUCLEO`/`FAKE_TERRITORIOS` indexados por mês/hora/tamanho
  de string, sem nenhuma requisição de rede. `kit.html` também não tinha
  nenhuma leitura de `natal_charts`, tabela e SVG eram 100% estáticos.
- **Implementado em `ritual-de-entrada.html`**: client Supabase
  (`@supabase/supabase-js@2` via jsdelivr), auth real (Google OAuth +
  e-mail/senha via `signUp`), geocoding real da cidade de nascimento
  (Nominatim/OSM, sem chave), lookup de fuso horário por coordenadas
  (timeapi.io, sem chave) com cálculo do offset UTC específico da data
  de nascimento via `Intl.DateTimeFormat` (respeita DST histórico,
  mesma aproximação já aceita no motor pra fuso da janela do dia, ver
  seção 5), upsert real em `profiles`, invoke real de
  `compute-natal-chart`, e preview populado com o retorno real da
  function antes de avançar pro step de confirmação.
- **Implementado em `kit.html`**: client Supabase, leitura de
  `natal_charts` do usuário logado, tabela populada com dados reais
  (núcleo/fome/máscara/cicatriz + território/grau reais). Estados de
  loading, "sem kit ainda" (CTA pro ritual) e erro de carga tratados.
  SVG da roda **continua estático**, renderizar posições reais por
  rotação de longitude ficou fora do escopo deste fix, é o próximo
  passo natural.
- **Assunções que ficam registradas, não são decisão fechada**:
  - `signUp` trata o caso de confirmação de e-mail estar ligada no
    Supabase Auth (guarda dados em `sessionStorage` e retoma sozinho
    quando o usuário volta logado), não sei qual é a configuração real
    do projeto no Dashboard, alguém confirma?
  - **"Território de abertura"** (copy do preview do ritual) foi
    mapeado pra território/casa do Núcleo (Sol), esse termo não existe
    no glossário nem tem regra de cálculo definida em lugar nenhum, foi
    chute educado pra não deixar o campo quebrado. Precisa virar entrada
    formal do glossário ou ser trocado por outra lógica.
  - Geocoding (Nominatim) e timezone (timeapi.io) são APIs públicas sem
...
- **Não coberto ainda**: se novas páginas forem adicionadas ao site, os
  links internas devem nascer sem `.html` desde já, seguindo esse padrão, não reintroduzir a extensão em código novo.
